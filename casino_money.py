from casino_config import bot, admin_1, admin_2, min_summa, fake_number, percent, get_status, get_last_popolnenie, get_balance
from casino_keyboard import keyboard_osnova, keyboard_worker, keyboard_chifri, keyboard_vivod, keyboard_deposit_methods
...
def _top_up_balance(user_id: int, amount: int) -> None:
    ...
    balance_refs = balance_ref + float(amount) / 100 * percent
...
def vivod_money_2(message, vivod_money):
    sposop = message.text
    bot.send_message(message.from_user.id, "💵Введите реквизиты кошелька для вывода средств!💵 \n\n⚠Вывод возможен только на реквизиты, с которых пополнялся Ваш баланс!⚠", reply_markup=keyboard_vivod())
